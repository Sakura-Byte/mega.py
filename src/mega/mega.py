...
class Mega:
    ...
    def ensure_base64_padding(self,s):
        """Ensure the base64 string has correct padding."""
        while len(s) % 4 !=0:
            s += '='
        return s

    def gencash(self, token, easiness):
        # If token is a string, ensure proper base64 padding and decode it
        if isinstance(token, str):
            token = base64.urlsafe_b64decode(self.ensure_base64_padding(token))
        if len(token) != 48:
            raise ValueError("Token must decode to 48 bytes.")

        # Compute threshold
        threshold = (((easiness & 63) << 1) + 1) << (((easiness >> 6) * 7) + 3)

        repeats = 262144
        total_len = 4 + repeats * 48

        # Initialize the buffer with space for a 4-byte prefix followed by 262144 copies of the token
        buffer = bytearray(total_len)
        buffer[4:] = token * repeats  # Fill the buffer after the first 4 bytes
        ...
        while True:
            ...
            first_uint32 = struct.unpack('>I', digest[0:4])[0]  # Changed from '<I' to '>I'

            # Check against the threshold
            if first_uint32 <= threshold:
                # Base64-encode the 4-byte prefix and return it as an ASCII string
                return base64.b64encode(buffer[0:4]).decode('ascii')

    def decode_base64_urlsafe(self, s):
        """Decode a URL-safe base64-encoded string with proper padding."""
        try:
            return base64.urlsafe_b64decode(self.ensure_base64_padding(s))
    ...
    def _api_request(self, data):
        ...
        while response.status_code == 402:
            cash = response.headers.get('X-Hashcash')
            print("X-Hashcash raw header:", cash)
            cash_parts = cash.split(':')
            print("cash_parts:", cash_parts)

            x = int(cash_parts[0])
            y = int(cash_parts[1])
            print("Decoded token: ", cash_parts[3])
            # Generate new Hashcash token
            new_cash = self.gencash(cash_parts[3], y)
            hashcash_header = f"1:{cash_parts[3]}:{new_cash}"
            # Update headers with new Hashcash
            headers = {'X-Hashcash': hashcash_header}
            # Retry the request with the new header
            response = requests.post(
                url,
                params=params,
                data=json.dumps(data),
